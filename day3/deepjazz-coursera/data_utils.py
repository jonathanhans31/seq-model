...
chords, abstract_grammars = get_musical_data('data/original_metheny.mid',part=5)
...
def load_music_utils(part):
    chords, abstract_grammars = get_musical_data('data/original_metheny.mid', part)
    corpus, tones, tones_indices, indices_tones = get_corpus_data(abstract_grammars)
    N_tones = len(set(corpus))
...
def generate_music(inference_model, save_path, corpus = corpus, abstract_grammars = abstract_grammars, tones = tones, tones_indices = tones_indices, indices_tones = indices_tones, T_y = 10, max_tries = 1000, diversity = 0.5):
    """
    ...
    mf.open("output/"+save_path+".midi", 'wb')
    mf.write()
