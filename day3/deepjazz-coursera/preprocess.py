...
def __parse_midi(data_fn, part):
    # Parse the MIDI data for separate melody and accompaniment parts.
    midi_data = converter.parse(data_fn)
    # Get melody part, compress into single voice.
    # melody_stream = midi_data[5]     # For Metheny piece, Melody is Part #5.
    melody_stream = midi_data[part]     # For Metheny piece, Melody is Part #5.
    
    for itr,data in enumerate(midi_data):
        print(np.array(data.getElementsByClass(stream.Voice)))
        fp = data.write("midi",fp=str(itr).zfill(5)+"_data.mid")
    
    # # Merge all the melodies into single voice
    melodies = melody_stream.getElementsByClass(stream.Voice)
    melody_voice = melodies[0]
    for i, melody in enumerate(melodies):
        if i == 0: continue # Skip the first melody
        print("Merging",i)
        for j in melody:
    ...
    # # print(melody_voice, mmelody_voice)
    # np.save("original.npy",np.array(melody_voice))
    ...
    for part in full_stream:
        ...
        curr_part.append(part.getElementsByOffset(476-10, 548+10, includeEndBoundary=True))
...
def get_musical_data(data_fn, part):
    
    measures, chords = __parse_midi(data_fn, part)
...
def load_music_utils():
    ...
    return (X, Y)
'''
__parse_midi("data/original_metheny.mid",0)
# exit()